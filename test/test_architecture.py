...
class ArchitectureTests(unittest.TestCase):
    ...
    def test_created_guild_becomes_selected_sidebar_item_before_add_discover_download(self):
        ...
        self.assertIn('"X-App-SPA": "1"', nav)
        self.assertIn('history.pushState({ appSpa: true }', nav)
        self.assertIn('importReplacement(parsed, ".sidebarList__5e434")', nav)
        self.assertIn('importReplacement(parsed, ".page__5e434")', nav)
        self.assertIn('updatePersistentGuildRail(path, guild)', nav)
        self.assertNotIn('location.assign(href)', nav)
    ...
    def test_password_policy_is_shared_and_strong(self):
        ...
        app_source = web_source()
        installer = (PRIV_SCRIPTS / "install_admin.py").read_text(encoding="utf-8")
        register = (ASSET_UI_JS / "register-form.js").read_text(encoding="utf-8")
        ...
        self.assertIn("validate_password_strength(password, username=username", installer)
    ...
    def test_local_hostname_installer_is_unique_logged_and_fail_closed(self):
        ...
        self.assertIn('Start-Process -FilePath "powershell.exe" -Verb RunAs', script)
